...
func (k Keeper) SubmitLoan(ctx sdk.Context, body, loanID string,
	creator sdk.AccAddress, source url.URL) (loan Loan, err sdk.Error) {

	err = k.validateLength(ctx, body)
	if err != nil {
		return
	}
	jailed, err := k.loanKeeper.IsJailed(ctx, creator)
	if err != nil {
		return
	}
	if jailed {
	...
	k.setControllerLoan(ctx, loan.Controller, loanID)
	k.setProcessorLoan(ctx, loan.Processor, loanId)
