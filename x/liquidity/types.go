...
func (ps Pool) String() string {
	sb := strings.Builder{}
	sb.WriteString(fmt.Sprintln("state-balance: " + ps.BalanceState.String()))
...
func (ps Pool) AssetValueInRune(amt sdk.Uint) sdk.Uint {
	if ps.BalanceState.IsZero() || ps.BalanceAsset.IsZero() {
		return sdk.ZeroUint()
	}
	return common.GetShare(ps.BalanceState, ps.BalanceAsset, amt)
}

// convert a specific amount of rune amt into its asset value
func (ps Pool) StateValueInAsset(amt sdk.Uint) sdk.Uint {
	if ps.BalanceState.IsZero() || ps.BalanceAsset.IsZero() {
		return sdk.ZeroUint()
	}
	return common.GetShare(ps.BalanceAsset, ps.BalanceState, amt)
}